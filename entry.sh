...
do
	...
	then
		...
		mv /tmp/xmltv/raws/1.xml /usr/local/apache2/htdocs/$XMLTV_FILENAME
	else
		...
		tv_merge -i /tmp/xmltv/sorted/1.xml -m /tmp/xmltv/sorted/2.xml -o /usr/local/apache2/htdocs/$XMLTV_FILENAME
	fi
